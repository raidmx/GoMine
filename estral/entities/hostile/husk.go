package hostile

import (
	"github.com/EstralMC/GoMine/estral/entities"
	"github.com/EstralMC/GoMine/estral/utils/nbtconv"
	"github.com/EstralMC/GoMine/server/block/cube"
	"github.com/EstralMC/GoMine/server/world"
	"github.com/go-gl/mathgl/mgl64"
)

type Husk struct {
	entities.MobBase
}

func NewHusk(nametag string, pos mgl64.Vec3, hasai bool) *Husk {
	z := &Husk{}
	z.MobBase = entities.NewMobBase(z, pos, nametag, hasai)

	return z
}

func (*Husk) Type() world.EntityType {
	return HuskType{}
}

type HuskType struct{}

func (HuskType) EncodeEntity() string { return "minecraft:husk" }
func (HuskType) BBox(_ world.Entity) cube.BBox {
	return cube.Box(-0.49, 0, -0.49, 0.49, 2, 0.49)
}

func (HuskType) DecodeNBT(data map[string]any) world.Entity {
	z := NewHusk(nbtconv.Map[string](data, "Nametag"), nbtconv.MapVec3(data, "Pos"), nbtconv.Map[bool](data, "HasAI"))
	return z
}

func (HuskType) EncodeNBT(e world.Entity) map[string]any {
	z := e.(*Husk)
	return map[string]any{
		"Nametag": z.MobBase.NameTag(),
		"Pos":     nbtconv.Vec3ToFloat32Slice(z.Position()),
		"HasAI":   z.MobBase.HasAi(),
	}
}
