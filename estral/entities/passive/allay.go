package passive

import (
	"github.com/EstralMC/GoMine/estral/entities"
	"github.com/EstralMC/GoMine/estral/utils/nbtconv"
	"github.com/EstralMC/GoMine/server/block/cube"
	"github.com/EstralMC/GoMine/server/world"
	"github.com/go-gl/mathgl/mgl64"
)

type Allay struct {
	entities.MobBase
}

func NewAllay(nametag string, pos mgl64.Vec3, hasai bool) *Allay {
	z := &Allay{}
	z.MobBase = entities.NewMobBase(z, pos, nametag, hasai)

	return z
}

func (*Allay) Type() world.EntityType {
	return AllayType{}
}

type AllayType struct{}

func (AllayType) EncodeEntity() string { return "minecraft:allay" }
func (AllayType) BBox(_ world.Entity) cube.BBox {
	return cube.Box(-0.49, 0, -0.49, 0.49, 2, 0.49)
}

func (AllayType) DecodeNBT(data map[string]any) world.Entity {
	z := NewAllay(nbtconv.Map[string](data, "Nametag"), nbtconv.MapVec3(data, "Pos"), nbtconv.Map[bool](data, "HasAI"))
	return z
}

func (AllayType) EncodeNBT(e world.Entity) map[string]any {
	z := e.(*Allay)
	return map[string]any{
		"Nametag": z.MobBase.NameTag(),
		"Pos":     nbtconv.Vec3ToFloat32Slice(z.Position()),
		"HasAI":   z.MobBase.HasAi(),
	}
}
