...
// ViewEntityDeltaMovement ...
func (s *Session) ViewEntityDeltaMovement(e world.Entity, pos mgl64.Vec3, yaw, pitch float64, onGround bool) {
	id := s.entityRuntimeID(e)
	if id == selfEntityRuntimeID || s.entityHidden(e) {
		return
	}
...
func (s *Session) ViewEntityAction(e world.Entity, a world.EntityAction) {
	...
	case entity.JumpAction:
		s.writePacket(&packet.ActorEvent{
			EntityRuntimeID: s.entityRuntimeID(e),
			EventType:       packet.ActorEventJump,
		})
	case entity.SwimAction:
		s.writePacket(&packet.ActorEvent{
			EntityRuntimeID: s.entityRuntimeID(e),
			EventType:       packet.ActorEventStartSwimming,
		})
	case entity.StartAttacking:
		s.writePacket(&packet.ActorEvent{
			EntityRuntimeID: s.entityRuntimeID(e),
			EventType:       packet.ActorEventStartAttacking,
		})
	case entity.StopAttacking:
		s.writePacket(&packet.ActorEvent{
			EntityRuntimeID: s.entityRuntimeID(e),
			EventType:       packet.ActorEventStopAttacking,
		})
	...
	case entity.EatGrassAction:
		s.writePacket(&packet.ActorEvent{
			EntityRuntimeID: s.entityRuntimeID(e),
			EventType:       packet.ActorEventEatGrass,
		})
	}
}
